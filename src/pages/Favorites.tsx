...
const Favorites = () => {
  ...
  const favoriteMovies = list.filter(
    (item) => item.isBookmarked === true && item.category === 'Movie'
  );

  const favoriteTvSeries = list.filter(
    (item) => item.isBookmarked === true && item.category === 'TV Series'
  );

  const favoriteShows = [...favoriteMovies, ...favoriteTvSeries];

  const filteredList = favoriteShows.filter((item) =>
    ...
    <main>
      ...
      <div className='w-screen flex flex-col mt-6 px-6 md:w-322 md:ml-4'>
        ...
        <div className='grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-7 md:grid-cols-4 md:gap-8'>
          {(searchQuery === '' ? favoriteMovies : filteredList).map(
            (item, i) => (
              <div
      ...
      <div
        className={`${
          searchQuery !== ''
            ? 'hidden'
            : 'w-screen flex flex-col mt-6 px-6 md:w-322 md:ml-4'
        } `}
      >
        <h2 className='mb-8 text-white font-light text-xl xs:text-2xl sm:text-3xl'>
          Favorite TV Series
